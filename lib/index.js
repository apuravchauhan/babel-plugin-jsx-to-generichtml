    ...
    function processCallExp(path, state) {
        ...
        if (t.isCallExpression(exp) && exp.callee.property.name == 'map') {
            ...
            if (Array.isArray(exp.arguments) && exp.arguments[0].body && Array.isArray(exp.arguments[0].body.body)) {
                body = exp.arguments[0].body.body[0].argument;
            }
            ...
            var attributesArray = [pseudoDataAttr];
            if (type) {
                pseudoTypeAttr = t.jSXAttribute(t.jSXIdentifier('type'), t.stringLiteral(type));
                attributesArray.push(pseudoTypeAttr);
            }
            var pseudoOpen = t.jSXOpeningElement(pseudo, attributesArray, false);
            var pseudoClose = t.jSXClosingElement(t.jSXIdentifier('PLUGIN-LOOP'));
            var bodyArray = [];
            if (body) {
                bodyArray.push(body);
            }
            var pseudoEle = t.jSXElement(pseudoOpen, pseudoClose, bodyArray);
            path.replaceWith(pseudoEle);
